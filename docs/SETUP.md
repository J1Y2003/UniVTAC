...
| `sbatch: error: ... Batch job submission failed: Unspecified error` | This cluster's submit filter enforces site rules and rejects the job before it queues. Known rules: a job name **longer than 50 characters**, `MODEL_OUTPUT_DIR` set under `/rlwrld-unified-checkpoints`, a `--wckey`, and no `--cpus-per-task`/`--mem` (jobs take the node defaults). Check with `sbatch --test-only`, which runs the filter without queueing anything. |
