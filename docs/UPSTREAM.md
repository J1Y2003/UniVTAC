...
| Image colour order, **relative to the scene** | The encode side already transposed R and B: `dict_to_hdf5` hands Isaac Sim's **RGB** array to `cv2.imencode`, which treats its input as BGR. So the JPEG on disk has R and B exchanged, and `imdecode`'s BGR-ordered return **cancels it** -- the decoded array is already true RGB. Anything that then "converts BGR to RGB" breaks it. Ground truth: the UniVTAC paper's tables are **orange**; a blue table means the channels are swapped | paper figures + verified 2026-09-17 against eval renders (orange) vs converted training video (blue) |
