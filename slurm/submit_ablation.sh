#!/bin/bash
# Submit the full ablation sweep: both arms x all UniVTAC benchmark tasks.
#
...
for arm in ${ARMS}; do
  ...
  for task in ${TASKS}; do
    exports="ALL,ARM=${arm},TASK=${task},TASK_CONFIG=${TASK_CONFIG}"
    ...
    cmd=(sbatch --job-name="uv-${arm}-${task}" --export="${exports}" "${REPO_ROOT}/slurm/eval_ablation.sbatch")
    echo "${cmd[*]}"
    if [[ "${DRY_RUN:-0}" != "1" ]]; then
      "${cmd[@]}"
    fi
